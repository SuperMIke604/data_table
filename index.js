...
function showDataOverview() {
    ...
    try {
        ...
        for (let i = chat.length - 1; i >= 0; i--) {
            ...
            messageData = extractDataFromMessage(message);
...
async function handleDeleteMessage(e) {
    ...
    try {
        ...
        if (message.mes) {
            try {
                // 移除 <dataUpdate> 标签块
                message.mes = message.mes.replace(/<dataUpdate>[\s\S]*?<\/dataUpdate>/g, '');
...
async function handleDeleteAllRecords() {
    try {
        ...
        for (let i = 0; i < chat.length; i++) {
            const message = chat[i];
            if (extractDataFromMessage(message)) {
                dataCount++;
            }
        }
...
// ============================================================
//  extractDataFromMessage — 统一从消息中提取 <dataUpdate> JSON 数据
// ============================================================
 ...
 *
 * 提取优先级：
 *   1. message.TavernDB_ACU_Data  — 已保存的结构化数据（最可靠）
 *   2. <dataUpdate>{...}</dataUpdate>  — AI 返回的新格式标签
 *   3. 整段文本直接 JSON.parse  — 兜底
 *
 * @param {object} message - 聊天消息对象
 * @param {object} [options] - 选项
 * @param {boolean} [options.requireMate=false] - 是否要求包含 mate.type=chatSheets 才算有效
 * @returns {object|null} 解析出的数据对象，或 null
 */
function extractDataFromMessage(message, options = {}) {
    if (!message) return null;
    const { requireMate = false } = options;

    // 1. 优先取已保存的结构化数据
    if (message.TavernDB_ACU_Data) {
        return message.TavernDB_ACU_Data;
    }

    // 2. 从消息文本中提取
    if (!message.mes) return null;
    const mesText = message.mes.trim();
    ...
    // 2b. 兜底：整段文本直接 JSON.parse
    try {
        const json = JSON.parse(mesText);
        if (json && typeof json === 'object') {
            if (!requireMate || (json.mate && json.mate.type === 'chatSheets')) {
                return json;
            }
...
// ============================================================
//  parseDataUpdate — 解析 AI 返回的 <dataUpdate> 增量更新指令
// ============================================================

/**
 * 解析 AI 返回的 <dataUpdate> 块中的 JSON operations 数组，
 * 并应用到 currentJsonTableData。
 *
 * AI 输出格式：
 * <dataUpdate>
 * {
 *   "operations": [
 *     { "action": "insert", "sheet": "sheet_1", "row": 1, "data": {"0":"值","1":"值"} },
 *     { "action": "update", "sheet": "sheet_1", "row": 2, "data": {"0":"新值"} },
 *     { "action": "delete", "sheet": "sheet_1", "row": 3 }
 *   ]
 * }
 * </dataUpdate>
 *
 * 也支持完整数据替换格式（无 operations 字段时）：
 * <dataUpdate>
 * { "mate": {...}, "sheet_1": {...}, ... }
 * </dataUpdate>
 *
 * @param {string} aiResponse - AI 原始响应文本
 * @returns {boolean} 是否成功应用了操作
 */
function parseDataUpdate(aiResponse) {
    if (!currentJsonTableData) {
        console.error('parseDataUpdate: currentJsonTableData 未加载');
        return false;
    }

    // 提取 <dataUpdate> 块
    const match = aiResponse.match(/<dataUpdate>([\s\S]*?)<\/dataUpdate>/);
    if (!match || !match[1]) {
        console.warn('parseDataUpdate: AI 响应中未找到 <dataUpdate> 块');
        return false;
    }
    ...
    if (!data || typeof data !== 'object') {
        console.warn('parseDataUpdate: 解析结果不是有效对象');
        return false;
    }

    // 格式 A：operations 数组 — 增量更新
    if (Array.isArray(data.operations)) {
        return applyOperations(data.operations);
    }

    // 格式 B：完整数据替换 — 直接合并/覆盖 sheet_*
    const sheetKeys = Object.keys(data).filter(k => k.startsWith('sheet_'));
    if (sheetKeys.length > 0) {
        sheetKeys.forEach(key => {
            currentJsonTableData[key] = JSON.parse(JSON.stringify(data[key]));
        });
        showToast(`已从 <dataUpdate> 更新 ${sheetKeys.length} 张表格`, 'info');
        return true;
    }

    console.warn('parseDataUpdate: 数据中既没有 operations 也没有 sheet_ 表');
    return false;
}

/**
 * 应用 operations 数组中的操作到 currentJsonTableData
...
function applyOperations(operations) {
    ...
    operations.forEach((op, opIndex) => {
        if (!op || !op.action) {
            console.warn(`applyOperations: 第 ${opIndex} 项缺少 action 字段`, op);
            return;
        }

        try {
            // 支持 sheet 为数字索引或字符串 key
            let table;
            let tableIndex;
            if (typeof op.sheet === 'number') {
                tableIndex = op.sheet;
                table = sheets[tableIndex];
            } else if (typeof op.sheet === 'string' && op.sheet.startsWith('sheet_')) {
                table = currentJsonTableData[op.sheet];
                tableIndex = sheets.indexOf(table);
            } else if (typeof op.sheet === 'string') {
                // 按名称查找
                tableIndex = sheets.findIndex(t => t && t.name === op.sheet);
                table = tableIndex >= 0 ? sheets[tableIndex] : null;
            }

            if (!table || !table.content) {
                appendTableEditErrorLog({
                    reason: `${op.action} 目标表不存在`,
            ...
            switch (op.action) {
                case 'insert': {
                    const data = op.data || {};
                    const rowIndex = typeof op.row === 'number' ? op.row : null;
                    const newRow = [null];
                ...
                case 'delete': {
                    ...
                    if (typeof rowIndex === 'number' && table.content.length > rowIndex + 1) {
                ...
                case 'update': {
                    ...
                    const createEmptyRow = () => {
                        const row = [];
                        for (let i = 0; i < Math.max(1, headerLen); i++) row.push('');
                        row[0] = null;
                        return row;
                    };
                ...
                default:
                    console.warn(`applyOperations: 未知 action "${op.action}"`, op);
                    appendTableEditErrorLog({
                        reason: `未知 action: ${op.action}`,
                        command: JSON.stringify(op),
                    });
            }
        } catch (err) {
            console.error(`applyOperations: 第 ${opIndex} 项执行异常`, err);
            appendTableEditErrorLog({
                reason: '操作执行异常',
                command: JSON.stringify(op),
    ...
    if (appliedCount > 0) {
        showToast(`从 AI 响应中成功应用了 ${appliedCount} 个数据库更新`, 'info');
    }
...
function findPreviousDbMessage(chat, currentIndex) {
    for (let i = currentIndex - 1; i >= 0; i--) {
        ...
        const data = extractDataFromMessage(msg, { requireMate: true });
        if (data) {
            return { index: i, data };
        }
    }
...
async function showDataPreview() {
    ...
    try {
        ...
        for (let i = chat.length - 1; i >= 0; i--) {
            const message = chat[i];
            if (!message) continue;
            const data = extractDataFromMessage(message, { requireMate: true });
            if (data) {
                messageIndex = i;
                messageData = data;
                break;
...
async function proceedWithCardUpdate(messagesToUse, batchToastMessage = '正在填表，请稍候...', saveTargetIndex = -1) {
    ...
    try {
        ...
        for (let attempt = 1; attempt <= maxRetries; attempt++) {
            ...
            if (!aiResponse || !aiResponse.includes('<dataUpdate>') || !aiResponse.includes('</dataUpdate>')) {
                console.warn(`第 ${attempt} 次尝试失败：AI响应中未找到完整有效的 <dataUpdate> 标签`);
            ...
            const parseSuccess = parseDataUpdate(aiResponse);
            if (!parseSuccess) {
...
async function updateReadableLorebookEntry(createIfNeeded = false) {
    ...
    const SHEET_ENTRY_PREFIX = 'TavernDB-Sheet-';
    const summaryLimit = currentSettings.summaryTableMaxEntries || 10;

    try {
        const allEntries = await TavernHelper_API.getLorebookEntries(primaryLorebookName);

        // --- 1. 清理旧格式条目（兼容迁移） ---
        const oldPrefixes = ['TavernDB-ACU-ReadableDataTable', 'TavernDB-ACU-OutlineTable', 'TavernDB-ACU-SummaryTable', '重要人物条目', '总结条目', '小总结条目'];
        const oldUids = allEntries
            .filter(e => e?.comment && oldPrefixes.some(p => e.comment.startsWith(p)))
            .map(e => e.uid);
        if (oldUids.length > 0) {
            await TavernHelper_API.deleteLorebookEntries(primaryLorebookName, oldUids);
            console.log(`[世界书] 已清理 ${oldUids.length} 个旧格式条目`);
        }

        // --- 2. 收集当前所有 sheet 名称 ---
        const tableKeys = Object.keys(currentJsonTableData).filter(k => k.startsWith('sheet_'));
        const currentSheetNames = new Set();

        // --- 3. 每个 sheet 生成/更新一个世界书条目 ---
        for (const sheetKey of tableKeys) {
            const table = currentJsonTableData[sheetKey];
            if (!table || !table.name || !Array.isArray(table.content) || table.content.length < 2) {
                continue;
            }

            const tableName = table.name.trim();
            currentSheetNames.add(tableName);
            const entryComment = `${SHEET_ENTRY_PREFIX}${tableName}`;

            const headerRow = Array.isArray(table.content[0]) ? table.content[0].slice(1) : [];
            let rows = table.content.slice(1);
            let startIndex = 0;

            // 总结表截断：仅保留最新 N 行
            if (tableName === '总结表' && rows.length > summaryLimit) {
                startIndex = rows.length - summaryLimit;
                rows = rows.slice(-summaryLimit);
            }

            const readableContent = formatTableToReadable(tableName, headerRow, rows, startIndex);
            if (!readableContent) continue;

            const finalContent = `<${tableName}>\n\n${readableContent}\n\n</${tableName}>`;

            // 查找已有条目
            const existingEntry = allEntries.find(e => e.comment === entryComment);

            if (existingEntry) {
                if (existingEntry.content !== finalContent) {
                    await TavernHelper_API.setLorebookEntries(primaryLorebookName, [{
                    ...
                    console.log(`[世界书] 已更新: ${tableName}`);
                }
            } else if (createIfNeeded) {
                await TavernHelper_API.createLorebookEntries(primaryLorebookName, [{
                ...
                console.log(`[世界书] 已创建: ${tableName}`);
            }
        }

        // --- 4. 删除已不存在的 sheet 对应的旧条目 ---
        const staleUids = allEntries
            .filter(e => e?.comment && e.comment.startsWith(SHEET_ENTRY_PREFIX) && !currentSheetNames.has(e.comment.slice(SHEET_ENTRY_PREFIX.length)))
            .map(e => e.uid);
        if (staleUids.length > 0) {
            await TavernHelper_API.deleteLorebookEntries(primaryLorebookName, staleUids);
            console.log(`[世界书] 已删除 ${staleUids.length} 个过期条目`);
        }
