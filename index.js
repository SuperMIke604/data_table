...
const DEFAULT_SETTINGS = {
    ...
    wbUpdateConfig: {
        enabled: false,                // 总开关
        triggerMode: 'manual',         // 'manual' | 'withDbUpdate' | 'interval'
        intervalFloors: 5,             // 间隔楼层数（interval模式）
        startAfterFloors: 3,           // 起始楼层（interval模式）
        confirmDelete: true,           // 删除操作需确认
        maxCreatePerRound: 10,         // 单次最多创建条目数
        patchDuplicateGuard: true,     // patch追加去重
        contextMode: 'full',           // 'full' | 'summary'
        maxContentChars: 0,            // 内容截断（0=不限）
        reviewDepth: 10,               // 审查最近N条消息
    ...
    const popupHtml = `
        <div class="data-manage-popup">
            ...
            <div class="data-manage-tabs-nav">
                ...
                <button class="data-manage-tab-button" data-tab="wbupdate">世界书更新</button>
            </div>
            ...
            <div id="data-manage-tab-wbupdate" class="data-manage-tab-content">
                <div class="data-manage-card">
                    ...
                    <div class="data-manage-checkbox-group">
                        ...
                        <label for="wb-update-enabled">启用世界书AI更新</label>
                    </div>
                    <div style="margin-top:10px;">
                        ...
                        <select id="wb-update-trigger-mode" style="width:100%;padding:8px;margin-top:4px;border-radius:6px;border:1px solid var(--ios-border);">
                            ...
                            <option value="withDbUpdate">跟随数据表更新</option>
                            <option value="interval">独立间隔触发（每N楼）</option>
                        </select>
                    </div>
                ...
                <div class="data-manage-card">
                    ...
                    <div style="margin-top:10px;">
                        ...
                        <select id="wb-update-context-mode" style="width:100%;padding:8px;margin-top:4px;border-radius:6px;border:1px solid var(--ios-border);">
                            <option value="full">全量（包含完整条目内容）</option>
                            <option value="summary">摘要（仅列出条目名和属性）</option>
                        </select>
                    </div>
                    <div style="margin-top:10px;">
                        <label>内容截断字符数（0=不限）:</label>
                    ...
                    <div class="data-manage-checkbox-group" style="margin-top:10px;">
                        ...
                        <label for="wb-update-patch-dedup">Patch追加去重</label>
                    </div>
                    <div class="data-manage-checkbox-group">
                        <input type="checkbox" id="wb-update-confirm-delete" checked>
                        <label for="wb-update-confirm-delete">删除操作需确认</label>
                    </div>
                    <div class="data-manage-checkbox-group">
                        <input type="checkbox" id="wb-update-exclude-constant">
                        <label for="wb-update-exclude-constant">排除永久条目</label>
                    </div>
                    <button id="wb-update-save-rules" class="secondary" style="margin-top:10px;width:100%;">保存更新规则</button>
                </div>
                <div class="data-manage-card">
                    <h3>世界书更新预设</h3>
                    <div class="data-manage-button-group" style="margin-bottom:10px;">
                        <select id="wb-update-prompt-selector" style="flex:1;padding:8px;border-radius:6px;border:1px solid var(--ios-border);"></select>
                        <button id="wb-update-add-preset" class="secondary">+新增</button>
                        <button id="wb-update-delete-preset" class="secondary">删除</button>
                        <button id="wb-update-duplicate-preset" class="secondary">复制</button>
                    </div>
                    ...
                    <div class="data-manage-button-group" style="margin-top:10px;">
                        <button id="wb-update-add-segment" class="secondary">+添加消息段</button>
                        <button id="wb-update-save-preset" class="primary">保存预设</button>
                    </div>
                    <p class="data-manage-notes">占位符: $0=数据表 $1=对话历史 $4=世界书(已有格式) $5=格式指令 $6=世界书条目上下文</p>
                </div>
                <div class="data-manage-card">
                    <h3>操作</h3>
                    <div class="data-manage-button-group">
                    ...
                    <div id="wb-update-pending-badge" style="margin-top:8px;display:none;">
                        <span style="background:#ff9500;color:white;padding:3px 10px;border-radius:12px;font-size:13px;">待审核: <span id="wb-update-pending-count">0</span> 条</span>
                    </div>
                </div>
                <div class="data-manage-card">
                    <h3>审核队列</h3>
                ...
                <div class="data-manage-card">
                    ...
                    <button id="wb-update-clear-log" class="secondary" style="margin-top:8px;">清空日志</button>
                </div>
            </div>
        </div>
    `;
    ...
    try {
        ...
        if (success) {
            ...
            if (currentSettings.wbUpdateConfig?.enabled &&
                ...
                try {
                    console.log('[世界书更新] 跟随数据表更新触发');
                    await callWbUpdateAI();
                } catch (e) {
                    console.error('[世界书更新] 跟随更新失败:', e);
                    showToast('世界书更新失败: ' + e.message, 'error');
                }
            }
        }
...
function getWbUpdateFormatGuide() {
    return `当你判断需要更新世界设定时，请严格按以下JSON格式输出，不要附加任何额外文字：
...
function parseWorldUpdateCommands(text) {
    ...
    try {
        ...
        // 健壮的 JSON 解析：修复常见问题
        jsonText = jsonText.replace(/,\s*([}\]])/g, '$1'); // 移除尾部逗号
        let arr;
        try {
        ...
        console.warn('[世界书更新] 解析JSON失败:', e.message);
        return [];
    }
}
        ...
        try {
            ...
            switch (opType) {
                ...
                case 'insert_after': {
                    ...
                    if (idx === -1) { result.skipped++; result.errors.push(`insert_after: 未找到锚点 "${anchor}"`); break; }
                ...
                case 'insert_before': {
                    ...
                    const content = entry.content || '';
                    const idx = content.indexOf(anchor);
                    if (idx === -1) { result.skipped++; result.errors.push(`insert_before: 未找到锚点 "${anchor}"`); break; }
                ...
                case 'replace_text': {
                    ...
                    const content = entry.content || '';
                    if (!content.includes(find)) { result.skipped++; result.errors.push(`replace_text: 未找到 "${find}"`); break; }
                ...
                case 'remove_text': {
                    ...
                    if (!find) { result.skipped++; break; }
                    const content = entry.content || '';
                    if (!content.includes(find)) { result.skipped++; result.errors.push(`remove_text: 未找到 "${find}"`); break; }
                ...
                case 'set_field': {
                    ...
                    if (!allowed.includes(field)) { result.skipped++; result.errors.push(`set_field: 不允许修改 "${field}"`); break; }
                    entry[field] = val;
            ...
            result.skipped++;
            result.errors.push(`${op.op} 执行失败: ${e.message}`);
        }
    }
    return result;
}
...
async function buildWbUpdateContext(bookName) {
    ...
    if (!TH) return '（无法获取TavernHelper API，世界书为空）';
    try {
        const entries = await TH.getLorebookEntries(bookName);
        if (!entries || entries.length === 0) return '（世界书为空，尚无任何条目）';
        const cfg = currentSettings.wbUpdateConfig || {};
        ...
        if (cfg.contextMode === 'summary') {
            // 摘要模式
            const lines = [`共 ${vis.length} 个条目：`];
            vis.forEach((e, i) => {
                const mode = e.constant ? '🔵永久' : '🟢关键词';
                const st = (e.enabled !== false && !e.disable && !e.disabled) ? '启用' : '禁用';
                const dep = e.depth != null ? e.depth : (e.context || '?');
                lines.push(`${i + 1}. ${wbGetEntryDisplayName(e)} [${mode} ${st} d:${dep} o:${e.order || 200}] keys:${wbGetEntryKeys(e) || '无'}`);
            });
            return lines.join('\n');
        }
        ...
        vis.forEach((e, i) => {
            ...
            const st = isEnabled ? '✅启用' : '❌禁用';
            const dep = e.depth != null ? e.depth : (e.context || '?');
            const uid = e.uid || '?';

            lines.push('');
            lines.push(`── 条目 #${i + 1} ──`);
            lines.push(`名称: ${wbGetEntryDisplayName(e)}`);
            lines.push(`状态: ${mode} ${st} | 深度:${dep} 排序:${e.order || 200} UID:${uid}`);
            lines.push(`主关键词: ${wbGetEntryKeys(e) || '（无）'}`);

            const skeys = e.keysecondary || e.secondary_keys;
            if (skeys) {
                const skStr = Array.isArray(skeys) ? skeys.join(', ') : String(skeys);
                if (skStr) lines.push(`副关键词: ${skStr}`);
            }

            let content = e.content || '（空）';
        ...
        console.error('[世界书更新] 构建上下文失败:', error);
        return '（构建上下文失败: ' + error.message + '）';
    }
}
...
function wbPendingAdd(floor, commands) {
    ...
    wbPendingSave();
    console.log(`[世界书更新] 添加审核批次: ${batch.id}, ${commands.length}条指令`);
    return batch;
}
    ...
    try {
        ...
        if (raw) {
            ...
            if (Array.isArray(parsed)) {
                ...
                console.log(`[世界书更新] 恢复 ${parsed.length} 个审核批次`);
            }
        }
    } catch (e) {
        console.warn('[世界书更新] 恢复审核队列失败:', e);
    }
}
...
async function executeWbUpdateCommands(commands, bookName) {
    const TH = getTavernHelperAPI();
    if (!TH) {
        showToast('TavernHelper API不可用，无法写入世界书', 'error');
        return { ok: 0, err: commands.length, errors: ['TavernHelper不可用'] };
    }
    const cfg = currentSettings.wbUpdateConfig || {};
    ...
    try {
        ...
        return { ok: 0, err: commands.length, errors: [`加载条目失败: ${e.message}`] };
    }
            ...
            switch (cmd.action) {
                case 'create': {
                    if (createCount >= (cfg.maxCreatePerRound || 10)) {
                        skip++;
                        errors.push(`${cmd.entry_name}: 超过单次创建上限`);
                        break;
                    }
                    // 检查是否已存在
                ...
                case 'update': {
                    ...
                    if (!target) {
                        err++;
                        errors.push(`${cmd.entry_name}: 条目不存在`);
                        break;
                    }
                ...
                case 'patch': {
                    ...
                    if (!target) {
                        err++;
                        errors.push(`${cmd.entry_name}: 条目不存在（patch）`);
                        break;
                    }
...
async function callWbUpdateAI() {
    if (wbUpdateProcessing) {
        showToast('世界书更新正在处理中，请稍候', 'warning');
        return;
    }

    const bookName = await wbGetTargetBookName();
    if (!bookName) {
        showToast('无目标世界书', 'error');
        return;
    }

    wbUpdateProcessing = true;
    showToast('🌍 世界书更新：正在分析...', 'info');

    try {
        // 构建上下文
        const wbContext = await buildWbUpdateContext(bookName);
        ...
        if (chat.length > 0) {
            ...
            for (let i = start; i < chat.length; i++) {
                ...
                const prefix = m.is_user ? name1 : (m.name || '角色');
        ...
        showToast('🌍 世界书更新：等待AI响应...', 'info');
        console.log('[世界书更新] 发送消息:', messages.length, '段');

        // 调用API（直接发送世界书更新消息，不经过数据表预设）
        const aiReply = await _callWbUpdateAPI(messages);

        // 解析
        const cmds = parseWorldUpdateCommands(aiReply);
        console.log(`[世界书更新] 解析到 ${cmds.length} 条指令`);

        if (cmds.length === 0) {
            showToast('🌍 世界书更新：无需更新', 'info');
            wbAppendLog({ floor: wbGetCurrentFloor(), action: 'review', detail: '无需更新' });
            return;
        }

        // 加入审核队列
        const batch = wbPendingAdd(wbGetCurrentFloor(), cmds);
        showToast(`📋 世界书更新：${cmds.length}条指令已加入审核队列`, 'info');

        // 尝试刷新UI
        wbRefreshPendingUI();

    } catch (e) {
        console.error('[世界书更新] 失败:', e);
        showToast(`❌ 世界书更新失败: ${e.message}`, 'error');
...
// 底层API调用（直接发送消息，不经过数据表预设）
async function _callWbUpdateAPI(messages) {
    const apiConfig = currentSettings.apiConfig || {};
    ...
    if (currentSettings.apiMode === 'tavern') {
        ...
        const context = SillyTavern.getContext();
        if (!context?.ConnectionManagerRequestService?.sendRequest) throw new Error('ConnectionManagerRequestService不可用');
        ...
        throw new Error(`酒馆预设API调用返回无效响应: ${response?.error || JSON.stringify(response)}`);
    } else {
        if (apiConfig.useMainApi) {
            const parentWin = (window.parent && window.parent !== window) ? window.parent : window;
            const TH = parentWin.TavernHelper || window.TavernHelper;
            if (!TH || typeof TH.generateRaw !== 'function') throw new Error('TavernHelper.generateRaw 不可用');
            const response = await TH.generateRaw({ ordered_prompts: messages, should_stream: false });
            if (typeof response !== 'string') throw new Error('主API未返回文本');
            return response.trim();
        } else {
            if (!apiConfig.url || !apiConfig.model) throw new Error('API URL或模型未配置');
            const context = SillyTavern.getContext();
...
function wbCheckIntervalTrigger() {
    ...
    if (wbIsUpdateDue(currentFloor)) {
        console.log(`[世界书更新] 间隔触发: 第${currentFloor}楼`);
        callWbUpdateAI();
    }
}
...
function renderWbPendingQueue(container, parentDoc) {
    ...
    if (batches.length === 0) {
        container.innerHTML = '<div class="wb-pending-empty">暂无待审核的更新指令</div>';
        return;
    }
    ...
    batches.forEach(batch => {
        ...
        html += `<div class="wb-pending-batch" data-batch-id="${batch.id}">
            <div class="wb-pending-batch-header">
                <span>📋 AI第${batch.floor}楼 · ${batch.commands.length}条指令 · ${time}</span>
                <div class="wb-pending-batch-actions">
                    <button class="wb-btn wb-btn-approve-all" data-batch="${batch.id}">✅ 全部批准</button>
                    <button class="wb-btn wb-btn-reject-all" data-batch="${batch.id}">❌ 全部拒绝</button>
                </div>
            </div>`;

        batch.commands.forEach((cmd, cmdIdx) => {
            const icon = { create: '✏️', update: '🔄', delete: '🗑️', patch: '🔧' }[cmd.action] || '❓';
            const edited = cmd._edited ? ' <span class="wb-edited-tag">已编辑</span>' : '';
            ...
            html += `<div class="wb-pending-cmd" data-batch="${batch.id}" data-cmd-idx="${cmdIdx}">
                <div class="wb-pending-cmd-header">
                    ...
                    <div class="wb-pending-cmd-actions">
                        <button class="wb-btn wb-btn-sm wb-btn-approve-one" data-batch="${batch.id}" data-idx="${cmdIdx}">✅</button>
                        <button class="wb-btn wb-btn-sm wb-btn-reject-one" data-batch="${batch.id}" data-idx="${cmdIdx}">❌</button>
                        ${cmd.action !== 'delete' ? `<button class="wb-btn wb-btn-sm wb-btn-edit" data-batch="${batch.id}" data-idx="${cmdIdx}">✏️</button>` : ''}
                    </div>
                </div>`;
            ...
            if (cmd.action === 'create' || cmd.action === 'update') {
                html += `<div class="wb-pending-cmd-detail">
                    <div class="wb-field-row"><span class="wb-field-label">内容:</span><div class="wb-field-value">${escapeHtml((cmd.fields.content || '').substring(0, 300))}</div></div>
                    ${cmd.fields.keys ? `<div class="wb-field-row"><span class="wb-field-label">关键词:</span><span class="wb-field-value">${escapeHtml(typeof cmd.fields.keys === 'string' ? cmd.fields.keys : JSON.stringify(cmd.fields.keys))}</span></div>` : ''}
                </div>`;
            ...
            // 编辑区（默认隐藏）
            html += `<div class="wb-edit-area" id="wb-edit-${batch.id}-${cmdIdx}" style="display:none;">`;
            if (cmd.action === 'create' || cmd.action === 'update') {
                html += `
                    <label>条目名称: <input type="text" class="wb-edit-name" value="${escapeHtml(cmd.entry_name)}" /></label>
                    <label>内容: <textarea class="wb-edit-content" rows="6">${escapeHtml(cmd.fields.content || '')}</textarea></label>
                    <label>关键词: <input type="text" class="wb-edit-keys" value="${escapeHtml(typeof cmd.fields.keys === 'string' ? cmd.fields.keys : (Array.isArray(cmd.fields.keys) ? cmd.fields.keys.join(', ') : ''))}" /></label>
                    <button class="wb-btn wb-btn-save-edit" data-batch="${batch.id}" data-idx="${cmdIdx}">💾 保存编辑</button>`;
            } else if (cmd.action === 'patch') {
                cmd.ops.forEach((op, oi) => {
                    html += `<div class="wb-edit-op" data-op-idx="${oi}">
                        <label>操作: <select class="wb-edit-op-type">
                        ...
                        ${op.anchor !== undefined ? `<label>锚点: <input type="text" class="wb-edit-op-anchor" value="${escapeHtml(op.anchor || '')}" /></label>` : ''}
                        ${op.find !== undefined ? `<label>查找: <input type="text" class="wb-edit-op-find" value="${escapeHtml(op.find || '')}" /></label>` : ''}
                        ${op.field !== undefined ? `<label>字段: <input type="text" class="wb-edit-op-field" value="${escapeHtml(op.field || '')}" /></label>` : ''}
                        <label>值: <textarea class="wb-edit-op-value" rows="2">${escapeHtml(op.value !== undefined ? String(op.value) : '')}</textarea></label>
                    </div>`;
                });
                html += `<button class="wb-btn wb-btn-save-edit" data-batch="${batch.id}" data-idx="${cmdIdx}">💾 保存编辑</button>`;
            }
    ...
    container.querySelectorAll('.wb-btn-approve-all').forEach(btn => {
        btn.addEventListener('click', async () => {
            ...
            showToast(`✅ 已执行: ${results.ok}成功, ${results.err}失败, ${results.skip || 0}跳过`, results.err > 0 ? 'warning' : 'success');
            wbRefreshPendingUI();
        });
    });
    ...
    container.querySelectorAll('.wb-btn-approve-one').forEach(btn => {
        btn.addEventListener('click', async () => {
            ...
            if (results) showToast(`✅ ${results.ok}成功`, results.err > 0 ? 'warning' : 'success');
            wbRefreshPendingUI();
        });
    });
...
function setupWbUpdateTabListeners(parentDoc) {
    // 启用开关
    const enabledCb = parentDoc.getElementById('wb-update-enabled');
    if (enabledCb) {
        enabledCb.addEventListener('change', function () {
            ...
            saveSettings();
            showToast(this.checked ? '已启用世界书AI更新' : '已禁用世界书AI更新', 'info');
        });
    }
    ...
    if (clearLogBtn) {
        clearLogBtn.addEventListener('click', function () {
            ...
            if (logContainer) logContainer.innerHTML = '<em>无日志</em>';
    ...
    if (addPresetBtn) {
        addPresetBtn.addEventListener('click', function () {
            const name = prompt('请输入新预设的名称:', `世界书预设 ${(currentSettings.wbUpdatePrompts || []).length + 1}`);
    ...
    if (deletePresetBtn) {
        deletePresetBtn.addEventListener('click', function () {
            ...
            const idx = currentSettings.wbUpdateCurrentPromptIndex || 0;
            if (confirm(`确定要删除预设"${currentSettings.wbUpdatePrompts[idx]?.name}"吗？`)) {
    ...
    if (duplicatePresetBtn) {
        duplicatePresetBtn.addEventListener('click', function () {
            ...
            if (!src) return;
            const name = prompt('请输入复制预设的名称:', `${src.name} 副本`);
            if (!name || !name.trim()) return;
    ...
    // 添加消息段
    const addSegBtn = parentDoc.getElementById('wb-update-add-segment');
    if (addSegBtn) {
...
function loadWbUpdateTabUI(parentDoc) {
    ...
    if (logContainer) {
        ...
        if (logs.length === 0) {
            logContainer.innerHTML = '<em>无日志</em>';
        } else {
            logContainer.innerHTML = logs.slice(0, 50).map(l => {
                const t = new Date(l.time).toLocaleString();
                const detail = l.detail || (l.results ? `成功:${l.results.ok} 失败:${l.results.err}` : '');
...
function _createWbSegmentHtml(index, segment) {
    ...
    return `<div class="wb-prompt-segment" data-seg-idx="${index}" style="margin-bottom:10px;padding:10px;border:1px solid var(--ios-border);border-radius:6px;background:var(--ios-gray);">
        <div style="display:flex;align-items:center;gap:8px;margin-bottom:6px;">
            <span style="font-weight:600;font-size:12px;">段 #${index + 1}</span>
            <select class="wb-seg-role" style="padding:4px 8px;border-radius:4px;border:1px solid var(--ios-border);">${roleSel}</select>
            <button class="wb-seg-delete secondary" style="margin-left:auto;font-size:12px;padding:2px 8px;">删除</button>
        </div>
    ...
    container.querySelectorAll('.wb-seg-delete').forEach(btn => {
        btn.onclick = function () {
            ...
            container.querySelectorAll('.wb-prompt-segment').forEach((s, i) => {
                s.dataset.segIdx = i;
                s.querySelector('span').textContent = `段 #${i + 1}`;
            });
        };
    });
}
